document.addEventListener('DOMContentLoaded', () => {
    ...
    const firebaseConfig = {
        ...
        storageBucket: "monitoramento-da-rede-ifpruv.appspot.com", // Corrigido o domínio
    ...
    try {
        ...
        document.body.innerHTML = '<h1 style="color:red; text-align:center;">Erro Crítico: Não foi possível conectar ao Firebase. Verifique a configuração e o console.</h1>';
        return;
    }
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            userInfo.style.display = 'flex'; // Usar flex para melhor alinhamento
    ...
    function renderDashboard(statuses) {
        ...
        if (Object.keys(statuses).length === 0 && auth.currentUser) {
    ...
    function showHistoryModal(deviceId) {
        ...
        historyRef.child(deviceId).limitToLast(1000).once('value', (snapshot) => {
            ...
            historyChart = new Chart(chartCanvas.getContext('2d'), {
                ...
                options: {
                    scales: {
                        x: { type: 'time', time: { unit: 'hour', displayFormats: { hour: 'HH:mm' } }, title: { display: true, text: 'Horário' } },
    ...
    // CÓDIGO NOVO (COM VALIDAÇÃO E FEEDBACK)

// Função auxiliar para validar um endereço IP usando expressão regular
...
deviceForm.addEventListener('submit', (e) => {
    ...
    if (!isValidIP(ipInput.value)) {
        ...
        ipInput.focus(); // Coloca o cursor no campo de IP
    ...
    const onComplete = (error) => {
        // Reabilita o botão, independentemente de ter dado erro ou sucesso
        submitBtn.disabled = false;
        submitBtn.textContent = 'Salvar Equipamento';

        if (error) {
            console.error('Falha ao salvar no Firebase:', error);
            alert('Erro ao salvar o equipamento. Tente novamente.');
        } else {
